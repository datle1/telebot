...
from telegram.ext import  Updater, CommandHandler,CallbackContext,ApplicationBuilder,ContextTypes, ConversationHandler,MessageHandler, filters
import sqlite3
import os
...
async def remove_func(update,context):
    username = update.message.from_user.username
    if username in ALLOWED_USERNAMES:
        user_name_list = context.args
...
async def insert_func (update, context):
    username = update.message.from_user.username
    if username in ALLOWED_USERNAMES:
        user_name_list = context.args
        if len(user_name_list) > 0:
...
async def show(update,context):
    ...
    if username in ALLOWED_USERNAMES:
        ...
        # Notify unauthorized user
        await update.message.reply_text("You are not authorized to use this bot.")
"6652724069:AAH5_kzdguWLCeSP1ncrEzs5x02D9Zt9xyw"
...
async def show_job(update: Update, context: CallbackContext) -> int:
    ...
    if jobs and len(jobs[0]) >= 5:
        job_list = '\n'.join([f"ID: {job[0]}, Ngày: {job[1]}, Công việc: {job[2]}, Thời gian: {job[3]}, Trạng thái: {status[job[4]]}" for job in jobs])
...
async def add_job(update: Update, context: CallbackContext) -> int:
    ...
    if step == 'day':
        ...
        if all(validate_day(day) for day in days):
            ...
            await update.message.reply_text(f"Nhập thông tin về công việc bạn muốn thêm hoặc nhập 'done' để kết thúc:")
        ...
        # else:
        #     await update.message.reply_text("Thông điệp không hợp lệ. Nhập lại thông điệp:")
        ...
        if all(validate_time(time) for time in times):
            ...
            context.user_data['step'] = 'done'  # Set the step to 'done'
            # return ConversationHandler.END
        else:
        ...
        cursor.execute('INSERT INTO job (days, message, times, status) VALUES (?, ?, ?, ?)',
                        (days_str, message, times_str, True))
        conn.commit()
...
async def help (update,context):
    await update.message.reply_text("Bot Nhắc việc:\n/help: Thông tin các lệnh\n/show: Hiển thị danh sách thành viên\n/insert <tên các thành viên>: Thêm thành viên vào danh sách\n/remove <tên các thành viên>: Xóa các thành viên khỏi danh sách\n/show_job: Bắt đầu quá trình thêm công việc cần nhắc và hiện thị danh sách công việc")

SHOW_JOB, MODIFY_JOB, ADD_JOB, REMOVE_JOB, SWITCH_JOB = range(5)
...
def main() -> None:
    ...
    insert_handler = CommandHandler("insert",insert_func)
    show_handler = CommandHandler("show",show)
    remove_handle = CommandHandler("remove",remove_func)
    shout_handle = CommandHandler("all",shout)
    help_hanlder = CommandHandler("help",help)

    not_command_filter = NotCommandFilter()

    # Add the states to the conversation handler
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler('show_job', show_job)],
        states={
    ...
    application.add_handler(remove_handle)
    application.add_handler(shout_handle)
